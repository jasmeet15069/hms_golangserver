...
func (h *BookingHandler) CheckAvailability(c *fiber.Ctx) error {
	...
	rows, err := h.pool.Query(c.Context(), `
		...
		GROUP BY d.date
		ORDER BY d.date`,
		postgres.DemoHotelID, checkIn, checkOut,
	)
	if err != nil {
...
func (h *BookingHandler) SearchRooms(c *fiber.Ctx) error {
	...
	args := []interface{}{postgres.DemoHotelID, req.CheckIn, req.CheckOut}
	argIdx := 4
...
func (h *BookingHandler) CreatePromotion(c *fiber.Ctx) error {
	...
	_, err := h.pool.Exec(c.Context(), `
		...
		promoID, postgres.DemoHotelID, req.Code, req.Name,
...
func (h *BookingHandler) TogglePromotion(c *fiber.Ctx) error {
	...
	tag, err := h.pool.Exec(c.Context(), `UPDATE promotions SET active = $1 WHERE id = $2 AND hotel_id = $3`, *req.Active, id, postgres.DemoHotelID)
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}
	if tag.RowsAffected() == 0 {
