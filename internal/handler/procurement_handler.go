...
func (h *ProcurementHandler) ListVendors(c *fiber.Ctx) error {
	q := `SELECT id, hotel_id, name, contact_person, email, phone, address,
	      ...
	      FROM vendors
	      WHERE hotel_id = $1`
	args := []interface{}{postgres.DemoHotelID}
	argIdx := 2
...
func (h *ProcurementHandler) CreateVendor(c *fiber.Ctx) error {
	...
	_, err := h.pool.Exec(c.Context(), `
		...
		vendorID, postgres.DemoHotelID, req.Name,
...
func (h *ProcurementHandler) UpdateVendor(c *fiber.Ctx) error {
	...
	if req.Active != nil && !*req.Active {
		_, err = h.pool.Exec(c.Context(), `UPDATE vendors SET active = false WHERE id = $1 AND hotel_id = $2`, id, postgres.DemoHotelID)
	} else {
		q := "UPDATE vendors SET "
		args := make([]interface{}, 0)
		argIdx := 1
		if req.Name != nil { q += fmt.Sprintf("name = $%d, ", argIdx); args = append(args, *req.Name); argIdx++ }
		if req.ContactPerson != nil { q += fmt.Sprintf("contact_person = $%d, ", argIdx); args = append(args, *req.ContactPerson); argIdx++ }
		if req.Email != nil { q += fmt.Sprintf("email = $%d, ", argIdx); args = append(args, *req.Email); argIdx++ }
		if req.Phone != nil { q += fmt.Sprintf("phone = $%d, ", argIdx); args = append(args, *req.Phone); argIdx++ }
		if req.Address != nil { q += fmt.Sprintf("address = $%d, ", argIdx); args = append(args, *req.Address); argIdx++ }
		if req.Category != nil { q += fmt.Sprintf("category = $%d, ", argIdx); args = append(args, *req.Category); argIdx++ }
		if req.Active != nil { q += fmt.Sprintf("active = $%d, ", argIdx); args = append(args, *req.Active); argIdx++ }
		q = q[:len(q)-2] + fmt.Sprintf(" WHERE id = $%d AND hotel_id = $%d", argIdx, argIdx+1)
		args = append(args, id, postgres.DemoHotelID)
		_, err = h.pool.Exec(c.Context(), q, args...)
	}
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}
...
func (h *ProcurementHandler) CreatePurchaseOrder(c *fiber.Ctx) error {
	...
	_, err = h.pool.Exec(c.Context(), `
		...
		poID, postgres.DemoHotelID, vendorID, poNumber,
...
func (h *ProcurementHandler) UpdatePOStatus(c *fiber.Ctx) error {
	...
	tag, err := h.pool.Exec(c.Context(), q, req.Status, id, postgres.DemoHotelID)
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}
