...
func (h *BillingHandler) GetFolio(c *fiber.Ctx) error {
	...
	chargeRows, err := h.pool.Query(c.Context(), `
	...
	payRows, err := h.pool.Query(c.Context(), `
...
func (h *BillingHandler) RecordPayment(c *fiber.Ctx) error {
	...
	err = h.pool.QueryRow(c.Context(), `SELECT booking_id FROM folios WHERE id = $1 AND hotel_id = $2`,
		folioID, postgres.DemoHotelID).Scan(&stayID)
	if err != nil {
		return response.Error(c, fiber.StatusNotFound, "folio not found")
	}
	...
	_, err = h.pool.Exec(c.Context(), `
		...
		paymentID, postgres.DemoHotelID, paymentNumber, stayID, req.Amount, method, notes)
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}

	h.audit(c, postgres.DemoHotelID, nil, "CREATE", "payments", paymentID, map[string]interface{}{
		"folio_id":       folioID,
...
func (h *BillingHandler) GenerateInvoice(c *fiber.Ctx) error {
	...
	err = h.pool.QueryRow(c.Context(),
		...
		folioID, postgres.DemoHotelID).Scan(&currency, &guestID)
	if err != nil {
		return response.Error(c, fiber.StatusNotFound, "folio not found")
	}
	...
	_, err = h.pool.Exec(c.Context(), `
		...
		invoiceID, postgres.DemoHotelID, folioID, invoiceNumber,
		subtotal, taxTotal, total, currency, nullableBillingText(req.Notes))
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}

	h.audit(c, postgres.DemoHotelID, &guestID, "CREATE", "invoices", invoiceID, map[string]interface{}{
		"folio_id":       folioID,
...
func (h *BillingHandler) GetInvoice(c *fiber.Ctx) error {
	...
	err = h.pool.QueryRow(c.Context(), `
		...
		JOIN folios f ON f.id = i.folio_id
		JOIN guest_stays gs ON gs.id = f.booking_id
		WHERE i.id = $1 AND i.hotel_id = $2`, id, postgres.DemoHotelID,
...
func (h *BillingHandler) EmailInvoice(c *fiber.Ctx) error {
	...
	err = h.pool.QueryRow(c.Context(), `
		...
		id, postgres.DemoHotelID).Scan(&invoiceNumber, &status, &folioID, &total, &guestEmail, &guestName)
	if err != nil {
		return response.Error(c, fiber.StatusNotFound, "invoice not found")
	}
	...
	h.audit(c, postgres.DemoHotelID, nil, "EMAIL", "invoices", id, map[string]interface{}{
		"invoice_number": invoiceNumber,
		"folio_id":       folioID,
	...
	go func() {
		defer func() { recover() }()
		_ = h.emailSvc.SendInvoice(guestEmail, guestName, "Grand Hotel Mumbai", invoiceNumber, fmt.Sprintf("%.2f", total), time.Now().UTC().Format("2006-01-02"))
	}()
