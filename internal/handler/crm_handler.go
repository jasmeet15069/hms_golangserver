...
func (h *CRMHandler) ListGuests(c *fiber.Ctx) error {
	q := `SELECT g.id, g.full_name, g.email, g.phone,
	             ...
	             COUNT(DISTINCT b.id) FILTER (WHERE b.status = 'checked_out') AS total_stays,
...
func (h *CRMHandler) GetGuest(c *fiber.Ctx) error {
	...
	stayRows, err := h.pool.Query(c.Context(), `
		SELECT b.id, r.room_number, b.check_in, b.check_out, b.total
		FROM bookings b
	...
	txRows, err := h.pool.Query(c.Context(), `
...
func (h *CRMHandler) UpdateLoyaltyTier(c *fiber.Ctx) error {
	...
	argIdx := 1
	if req.Name != "" { q += fmt.Sprintf("name = $%d, ", argIdx); args = append(args, req.Name); argIdx++ }
	if req.MinPoints != nil {
		if *req.MinPoints < 0 {
			return response.Error(c, fiber.StatusBadRequest, "min_points must be >= 0")
		}
		q += fmt.Sprintf("min_points = $%d, ", argIdx); args = append(args, *req.MinPoints); argIdx++
	}
	if req.Multiplier != nil { q += fmt.Sprintf("multiplier = $%d, ", argIdx); args = append(args, *req.Multiplier); argIdx++ }
	if req.Benefits != nil { q += fmt.Sprintf("benefits = $%d, ", argIdx); args = append(args, req.Benefits); argIdx++ }
	if argIdx == 1 {
		return response.Error(c, fiber.StatusBadRequest, "no fields to update")
	}
...
func (h *CRMHandler) AwardPoints(c *fiber.Ctx) error {
	...
	err = h.pool.QueryRow(c.Context(), `
		...
		RETURNING id`,
		postgres.DemoHotelID, guestID, req.Points,
	).Scan(&memberID)
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}
...
func (h *CRMHandler) RedeemPoints(c *fiber.Ctx) error {
	...
	_, err = h.pool.Exec(c.Context(),
		...
		req.Points, memberID, postgres.DemoHotelID,
	)
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}
