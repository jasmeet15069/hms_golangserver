...
func (h *HousekeepingHandler) CreateTask(c *fiber.Ctx) error {
	...
	_, err = h.pool.Exec(c.Context(), `
		...
		taskID, postgres.DemoHotelID, roomID, assignedTo, taskType, priority, notes,
	)
	if err != nil {
...
func (h *HousekeepingHandler) UpdateTask(c *fiber.Ctx) error {
	...
	argIdx++
	args = append(args, id)
	args = append(args, postgres.DemoHotelID)
	q := fmt.Sprintf(
...
func (h *HousekeepingHandler) CreateLostItem(c *fiber.Ctx) error {
	...
	_, err := h.pool.Exec(c.Context(), `
		...
		itemID, postgres.DemoHotelID, roomID, guestName, req.ItemName,
...
func (h *HousekeepingHandler) UpdateLostItem(c *fiber.Ctx) error {
	...
	_, err = h.pool.Exec(c.Context(), `
		...
		req.Status, time.Now().UTC(), id, postgres.DemoHotelID,
	)
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}
...
func (h *HousekeepingHandler) IssueLinen(c *fiber.Ctx) error {
	...
	_, err = h.pool.Exec(c.Context(), `
		...
		txID, postgres.DemoHotelID, linenID, req.Quantity,
	...
	_, err = h.pool.Exec(c.Context(), `
		...
		req.Quantity, linenID, postgres.DemoHotelID,
	)
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}
...
func (h *HousekeepingHandler) ReturnLinen(c *fiber.Ctx) error {
	...
	_, err = h.pool.Exec(c.Context(), `
		...
		req.Quantity, cleanQty, req.Damaged, linenID, postgres.DemoHotelID,
	)
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}
